...
DeepSeek-V4 series models are recommended for API-based usage, especially
`deepseek-v4-pro` and `deepseek-v4-flash`. For this system's multi-step
analysis, research-team debate, risk assessment, and long-form report
generation, DeepSeek-V4 provides strong reasoning quality at a very low API
cost.

Based on current practical usage, generating an approximately **50k-character**
multi-dimensional A-share trading analysis report costs about **RMB 0.2** in API
tokens. Actual cost depends on model version, cache hits, call count, report
length, and official pricing changes. Refer to the
...
| Analysis Area | Sources | Used For | Official Link / API Entry |
| --- | --- | --- | --- |
| Market Data | Tushare Pro + optional iFinD QuantAPI | OHLCV, valuation, money flow, limit data, technical indicators, and optional real-time quote enrichment | [Tushare Pro](https://tushare.pro), [Tushare token guide](https://tushare.pro/document/1?doc_id=39), [iFinD API examples](https://quantapi.51ifind.com/gwstatic/static/ds_web/quantapi-web/example.html), [iFinD help center](https://ftwc.51ifind.com/gwstatic/static/ds_web/quantapi-web/help-center.html) |
| News Flow | OpenNews MCP / REST + Jin10 MCP fallback | News search, market context, macro/policy context, and flash/news fallback | [OpenNews token portal](https://6551.io/mcp), [OpenNews MCP docs](https://github.com/6551Team/opennews-mcp/blob/main/docs/README_ZH.md), [Jin10 MCP docs](https://mcp.jin10.com/app/doc.html) |
| Fundamentals & Announcements | Tushare Pro + Cninfo/Juchao fallback for announcements | Company profile, financial statements, financial indicators, dividends, share float, forecasts, express reports, and announcements | [Cninfo](https://www.cninfo.com.cn/), [Cninfo WebAPI](https://webapi.cninfo.com.cn/#/apiDoc); see Market Data for Tushare |
| Social Sentiment | Eastmoney Guba + Tushare hotness + optional iFinD popularity signals | Authorized forum posts, Tushare `dc_hot` / `ths_hot`, and optional iFinD smart-stock-picking popularity signals | [Eastmoney Guba](https://guba.eastmoney.com.cn/); see Market Data for Tushare and iFinD |
